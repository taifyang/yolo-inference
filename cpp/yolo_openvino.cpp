...
void YOLO_OpenVINO_Classification::init(const Algo_Type algo_type, const Device_Type device_type, const Model_Type model_type, const std::string model_path)
{
...
void YOLO_OpenVINO_Detection::init(const Algo_Type algo_type, const Device_Type device_type, const Model_Type model_type, const std::string model_path)
{
	YOLO_OpenVINO::init(algo_type, device_type, model_type, model_path);
...
void YOLO_OpenVINO_Segmentation::init(const Algo_Type algo_type, const Device_Type device_type, const Model_Type model_type, const std::string model_path)
{
	YOLO_OpenVINO::init(algo_type, device_type, model_type, model_path);
...
void YOLO_OpenVINO_Classification::pre_process()
{
	cv::Mat crop_image;
	if (m_algo == YOLOv5)
	{
...
void YOLO_OpenVINO_Detection::pre_process()
{
	cv::Mat letterbox;
...
void YOLO_OpenVINO_Segmentation::pre_process()
{
	cv::Mat letterbox;
	LetterBox(m_image, letterbox, m_params, cv::Size(m_input_width, m_input_height));
	cv::dnn::blobFromImage(letterbox, m_input, 1. / 255., cv::Size(m_input_width, m_input_height), cv::Scalar(), true, false);
...
void YOLO_OpenVINO_Classification::process()
{
...
void YOLO_OpenVINO_Detection::process()
{
	ov::Tensor input_tensor(m_input_port.get_element_type(), m_input_port.get_shape(), m_input.ptr(0)); //Create tensor from external memory
	m_infer_request.set_input_tensor(input_tensor); // Set input tensor for model with one input
	m_infer_request.infer(); //Start inference
...
void YOLO_OpenVINO_Segmentation::process()
{
	ov::Tensor input_tensor(m_input_port.get_element_type(), m_input_port.get_shape(), m_input.ptr(0)); //Create tensor from external memory
	m_infer_request.set_input_tensor(input_tensor); // Set input tensor for model with one input
	m_infer_request.infer(); //Start inference
...
void YOLO_OpenVINO_Classification::post_process()
{
...
void YOLO_OpenVINO_Detection::post_process()
{
...
void YOLO_OpenVINO_Segmentation::post_process()
{
	std::vector<cv::Rect> boxes;
	std::vector<float> scores;
	std::vector<int> class_ids;
