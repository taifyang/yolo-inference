 ...
 * @LastEditTime: 2026-08-30 00:19:27
 * @Description: source file for YOLO tensorrt semantic segmentation
 */
...
void YOLO_TensorRT_Semantic::init(const Algo_Type algo_type, const Device_Type device_type, const Model_Type model_type, const std::string model_path)
{	
...
void YOLO_TensorRT_Semantic::pre_process()
{	
	YOLO_TensorRT_Detect::pre_process();
}

void YOLO_TensorRT_Semantic::process()
{
	m_execution_context->executeV2(m_bindings.data());

#ifndef _CUDA_POSTPROCESS
	cudaMemcpy(m_output0_host, m_output0_device, sizeof(uint8_t) * m_output_numdet, cudaMemcpyDeviceToHost);
#endif // !_CUDA_POSTPROCESS
}

void YOLO_TensorRT_Semantic::post_process()
{	
	m_mask = cv::Mat::zeros(m_image.size(), CV_8UC1);

#ifdef _CUDA_POSTPROCESS
	uint8_t* mask_device;
	cudaMalloc(&mask_device, sizeof(uint8_t) * m_image.cols * m_image.rows);
	cuda_scale_mask(m_output0_device, mask_device, m_input_size, m_image.size());
	cudaMemcpy(m_mask.data, mask_device, sizeof(uint8_t) * m_image.cols * m_image.rows, cudaMemcpyDeviceToHost);
...
void YOLO_TensorRT_Semantic::release()
{
